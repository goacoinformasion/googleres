...
def create_student_result(record: StudentResultSchema, db: Session = Depends(get_db)):
    db_record = db.query(StudentResultModel).filter(StudentResultModel.slno == record.slno).first()
    if db_record:
        return JSONResponse(
            status_code=400,
            content={
...
def get_result_by_slno(slno: int, db: Session = Depends(get_db)):
    record = db.query(StudentResultModel).filter(StudentResultModel.slno == slno).first()
    if not record:
...
def get_result_by_rollno(rollno: str, db: Session = Depends(get_db)):
    record = db.query(StudentResultModel).filter(StudentResultModel.rollno == rollno).first()
    if not record:
        raise HTTPException(status_code=404, detail="Record not found for this roll number")
    return record
